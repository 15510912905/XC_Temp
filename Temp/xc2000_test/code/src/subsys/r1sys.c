...
CR1Sys::CR1Sys(vos_u32 ulPid, vos_u32 ulDstSubsys) : CSubSys(ulPid, ulDstSubsys)
{
    _initParseFuc();
}
...
//初始化解析函数
void CR1Sys::_initParseFuc()
{
    m_parseFucs[Act_Reset] = &CSubSys::ParseNdlReset;
    m_parseFucs[Act_Move] = (ParseAct)&CR1Sys::_parseR1Rtt;
    m_parseFucs[Act_MoveAbs] = (ParseAct)&CR1Sys::_parseR1Rtt;
    m_parseFucs[Act_MoveW] = (ParseAct)&CR1Sys::_parseR1Rtt;
    m_parseFucs[Act_MoveDisg] = (ParseAct)&CR1Sys::_parseR1Rtt;
    m_parseFucs[Act_Absorb] = (ParseAct)&CR1Sys::_parseR1Absrb;
    m_parseFucs[Act_Discharge] = (ParseAct)&CR1Sys::_parseR1Discharge;
    m_parseFucs[Act_Wash] = (ParseAct)&CR1Sys::_parseR1Wash;
    m_parseFucs[Act_PVCtrl] = (ParseAct)&CR1Sys::_parsePVCtrl;
    m_parseFucs[Act_PumpCtrl] = (ParseAct)&CR1Sys::_parsePumpCtrl;
    m_parseFucs[Act_NdlVrt] = (ParseAct)&CR1Sys::_parseNdlVrt;
    m_parseFucs[Act_AirOut] = (ParseAct)&CR1Sys::_parseAirOut;

    m_parseFucs[Act_MarginScan] = (ParseAct)&CR1Sys::_parseR1MarginScan;
    m_parseFucs[Act_IntensifyWash] = (ParseAct)&CR1Sys::_parseIntensifyWash;
    m_parseFucs[Act_TrimOffset] = (ParseAct)&CR1Sys::_parseTrimOffset;
...
vos_u32 CR1Sys::_parseR1Rtt(msg_stComDevActInfo* pComAct)
{
...
vos_u32 CR1Sys::_parseR1Absrb(msg_stComDevActInfo* pComAct)
{
    app_u16 cmdNum = 3;
...
vos_u32 CR1Sys::_parseR1Discharge(msg_stComDevActInfo* pComAct)
{
    app_u16 cmdNum = 4;
    vos_u32 cmdLen = sizeof(msg_stComDwnmachCombCmd) + sizeof(msg_stComCombSingleCmd) * cmdNum
...
vos_u32 CR1Sys::_parseR1Wash(msg_stComDevActInfo* pComAct)
{
    app_u16 cmdNum = 2;
    vos_u32 cmdLen = sizeof(msg_stComDwnmachCombCmd) + sizeof(msg_stComCombSingleCmd) * cmdNum
...
vos_u32 CR1Sys::_parseR1MarginScan(msg_stComDevActInfo* pComAct)
{
    ...
    pCmd->cmd = cmd_r1_reagent_ndl_marginscan;

    return VOS_OK;
}

vos_u32 CR1Sys::_parseIntensifyWash(msg_stComDevActInfo* pComAct)
{
    ...
    pUp->cmd = cmd_r1_reagent_ndl_vrise;

    //回清洗位
    pSingleCmd = (msg_stComCombSingleCmd*)(((char*)pSingleCmd) + pSingleCmd->ulCmdSize + sizeof(vos_u32));
    pSingleCmd->ulCmdSize = sizeof(msg_stNdlHrotate);
    msg_stNdlHrotate* pToWashPos = (msg_stNdlHrotate*)(&(pSingleCmd->aucCmdLoad));
    pToWashPos->cmd = cmd_r1_reagent_ndl_hrotate_washpos;

    //强化清洗
    pSingleCmd = (msg_stComCombSingleCmd*)(((char*)pSingleCmd) + pSingleCmd->ulCmdSize + sizeof(vos_u32));
    pSingleCmd->ulCmdSize = sizeof(msg_stOneCmd);
    msg_stOneCmd* pIntensifyWash = (msg_stOneCmd*)(&(pSingleCmd->aucCmdLoad));
    pIntensifyWash->cmd = cmd_r1_ndl_intensify_wash;

    return VOS_OK;
}

vos_u32 CR1Sys::_parseAirOut(msg_stComDevActInfo* pComAct)
{
...
vos_u32 CR1Sys::_parseTrimOffset(msg_stComDevActInfo* pComAct)
{
    return ParseTrimOffset(pComAct,cmd_r1_ndl_step_move); 
}

vos_u32 CR1Sys::_parseNdlVrt(msg_stComDevActInfo* pComAct)
{
...
vos_u32 CR1Sys::_parsePVCtrl(msg_stComDevActInfo* pComAct)
{
...
vos_u32 CR1Sys::_parsePumpCtrl(msg_stComDevActInfo* pComAct)
{
...
uint8_t CR1Sys::_setPos(app_u16 usDstPos)
{
    if (m_ulPid == SPid_R1)
    {
        return (usDstPos == RNdlPosDischarg) ? PLACE_R11RTT_DISCHARGE
            : (usDstPos == RNdlPosReset) ? PLACE_R11RTT_RESET
            : (usDstPos == RNdlPosW1) ? PLACE_R11RTT_WASH1
            : (usDstPos == RNdlPosW2) ? PLACE_R11RTT_WASH2
            : (usDstPos == RNdlPosW3) ? PLACE_R11RTT_WASH3
            : (usDstPos == RNdlPosAbsorb1) ? PLACE_R11RTT_INHALATION1
            : (usDstPos == RNdlPosAbsorb2) ? PLACE_R11RTT_INHALATION2 : 0;
    }
    return (usDstPos == RNdlPosDischarg) ? PLACE_R12RTT_DISCHARGE
        : (usDstPos == RNdlPosReset) ? PLACE_R12RTT_RESET
        : (usDstPos == RNdlPosW1) ? PLACE_R12RTT_WASH1
        : (usDstPos == RNdlPosW2) ? PLACE_R12RTT_WASH2
        : (usDstPos == RNdlPosW3) ? PLACE_R12RTT_WASH3
        : (usDstPos == RNdlPosAbsorb1) ? PLACE_R12RTT_INHALATION1
        : (usDstPos == RNdlPosAbsorb2) ? PLACE_R12RTT_INHALATION2 : 0;
}
