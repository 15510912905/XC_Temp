...
CReagMix::CReagMix(vos_u32 ulPid) : CSubSys(ulPid, xc8002_dwnmach_name_reag_mixrod)
{
    _initParseFuc();
}
...
//初始化解析函数
void CReagMix::_initParseFuc()
{
    m_parseFucs[Act_Reset] = (ParseAct)&CReagMix::_parseReset;
    m_parseFucs[Act_Move] = (ParseAct)&CReagMix::_parseMove;
    m_parseFucs[Act_Mix] = (ParseAct)&CReagMix::_parseMix;
    m_parseFucs[Act_Wash] = (ParseAct)&CReagMix::_parseWash;
    m_parseFucs[Act_NdlVrt] = (ParseAct)&CReagMix::_parseNdlVrt;
    m_parseFucs[Act_PVCtrl] = (ParseAct)&CReagMix::_parsePVCtrl;
    m_parseFucs[Act_OlnyMix] = (ParseAct)& CReagMix::_parseOnlyMix;

    m_parseFucs[Act_TrimOffset] = (ParseAct)&CReagMix::_parseTrimOffset;
    ...
    m_parseFucs[Act_UpdateFpga] = (ParseAct)&CReagMix::_parseUpFpga;
}

vos_u32 CReagMix::_parseReset(msg_stComDevActInfo* pComAct)
{
...
vos_u32 CReagMix::_parseMove(msg_stComDevActInfo* pComAct)
{
...
vos_u32 CReagMix::_parseMix(msg_stComDevActInfo* pComAct)
{
    app_u16 cmdNum = 3;
...
vos_u32 CReagMix::_parseWash(msg_stComDevActInfo* pComAct)
{
    app_u16 cmdNum = 3;
    vos_u32 cmdLen = sizeof(msg_stComDwnmachCombCmd) + sizeof(msg_stComCombSingleCmd) * cmdNum
...
vos_u32 CReagMix::_parseTrimOffset(msg_stComDevActInfo* pComAct)
{
    return ParseTrimOffset(pComAct,cmd_reagent_mix_step_move); 
}

vos_u32 CReagMix::_parseNdlVrt(msg_stComDevActInfo* pComAct)
{
...
vos_u32 CReagMix::_parsePVCtrl(msg_stComDevActInfo* pComAct)
{
...
vos_u32 CReagMix::_parseOnlyMix(msg_stComDevActInfo* pComAct)
{
...
vos_u32 CReagMix::_parseUpFpga(msg_stComDevActInfo* pComAct)
{
    return ParseUpFpga(pComAct, cmd_reagent_mix_fpga_update);
}
